...
import { useMunicipalities } from "@/lib/use-municipalities";
...
function HomeContent() {
  ...
  const { geo: muniGeo, loading: muniLoading } = useMunicipalities();
  ...
  const filtered = useMemo(() => {
    if (!data) return [];
    const applied = applyFilters(data.outbreaks, {
      ...filters,
      dateFrom: timelineRange.from ?? filters.dateFrom,
      dateTo: timelineRange.to ?? filters.dateTo,
    });
    if (showOnlyActive) {
      return applied.filter((o) => o.status === "Ongoing");
    }
    return applied;
  }, [data, filters, showOnlyActive, timelineRange]);

  // Count of active outbreaks (for filter panel display)
  const onlyActiveCount = useMemo(() => {
    if (!data) return 0;
    return data.outbreaks.filter((o) => o.status === "Ongoing").length;
  }, [data]);
    ...
    <main className="flex h-dvh flex-col overflow-hidden bg-background">
              ...
              <SheetContent side="left" className="w-[85vw] max-w-sm overflow-y-auto thin-scroll pb-safe">
                <div className="p-4">
                  ...
                  <FilterPanel
                    ...
                    onlyActiveCount={onlyActiveCount}
                    showEnterprises={showEnterprises}
                    onShowEnterprisesChange={setShowEnterprises}
                  />
                </div>
              </SheetContent>
            </Sheet>
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <section className="relative min-h-0 flex-1 overflow-hidden">
          ...
          {/* Floating "Only active" toggle button — top-right on map */}
          <button
            onClick={() => setShowOnlyActive((v) => !v)}
            className={`absolute top-3 right-3 z-30 hidden md:flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium shadow-lg backdrop-blur-xl transition-all ${
              showOnlyActive
                ? "bg-red-500 text-white border-red-500"
                : "bg-card/60 text-foreground border-white/15 hover:bg-card/80"
            }`}
            title="Скрыть неактивные вспышки"
          >
            <span className={`h-1.5 w-1.5 rounded-full ${showOnlyActive ? "bg-white animate-pulse" : "bg-red-500"}`} />
            Только активные ({onlyActiveCount})
          </button>
          ...
          <div className="absolute bottom-3 right-3 z-20 max-w-[220px] rounded-2xl border border-white/15 bg-card/60 p-3 text-[10px] shadow-2xl backdrop-blur-xl pointer-events-auto">
            <div className="font-semibold text-foreground mb-1.5">Зоны риска (WOAH)</div>
            <LegendRow color="#dc2626" label="Защита (3 км) — запрет перемещения" />
            <LegendRow color="#f59e0b" label="Наблюдение (10 км) — ветсертификат" />
            <LegendRow color="#3b82f6" label="Ограничение (30 км) — мониторинг" />
            <div className="text-[9px] text-muted-foreground mt-1 pt-1 border-t border-white/10">
              Клик по зоне → информация о запрете
            </div>
            <div className="pt-1.5 mt-1.5 border-t border-white/10">
              <div className="font-semibold text-foreground mb-1">Terra incognita</div>
              <div className="flex items-center gap-1.5">
                <div className="w-4 h-0" style={{ borderTop: "2px dashed #374151" }} />
                <span className="text-foreground">Донецк, Луганск, Запорожье, Херсон</span>
              </div>
            </div>
            <div className="pt-1.5 mt-1.5 border-t border-white/10">
      ...
      <RegionDrillDown
        ...
        onOpenChange={(v) => {
          setRegionDrillDownOpen(v);
          // Сразу сбрасываем регион при закрытии Sheet —
          // мини-карта и таблица полностью размонтируются
          if (!v) {
