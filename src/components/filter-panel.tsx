...
interface FilterPanelProps {
  ...
  onlyActiveCount?: number;
  showEnterprises?: boolean;
  onShowEnterprisesChange?: (v: boolean) => void;
}
  ...
  return (
    <Card className="p-3 md:p-4 space-y-3">
      ...
      <div className="space-y-2 pt-2 border-t">
        ...
        {onShowOnlyActiveChange && (
          <div className="flex items-center justify-between">
            <Label htmlFor="only-active" className="text-xs cursor-pointer">
              Только активные вспышки{onlyActiveCount ? ` (${onlyActiveCount})` : ""}
            </Label>
            <Switch
              id="only-active"
        ...
        {onShowEnterprisesChange && (
          <div className="flex items-center justify-between">
            <Label htmlFor="enterprises" className="text-xs cursor-pointer">
              🏭 Реестр хозяйств
            </Label>
            <Switch
              id="enterprises"
              checked={showEnterprises}
              onCheckedChange={onShowEnterprisesChange}
            />
          </div>
        )}
      </div>
