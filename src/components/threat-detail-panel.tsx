...
export default function ThreatDetailPanel({ threat, onClose }: ThreatDetailPanelProps) {
  // Safe wrapper — prevent null/undefined array access crashes
  const safeThreat = {
    ...threat,
    preventionSteps: threat.preventionSteps ?? [],
    vaccines: threat.vaccines ?? [],
    recommendations: threat.recommendations ?? [],
    dataSources: threat.dataSources ?? [],
    affectedAnimals: threat.affectedAnimals ?? [],
    sources: threat.sources ?? [],
  };

  const effectiveLevel = getEffectiveThreatLevel(safeThreat);
  const config = threatLevelConfig[effectiveLevel];
  const isReal = safeThreat.isRealData;
  const status = isReal ? getOutbreakStatus(safeThreat) : null;
  const statusCfg = status ? outbreakStatusConfig[status] : null;
  const daysSince = getDaysSinceUpdate(safeThreat);
      ...
      <div className={`p-4 ${config.bgColor} border-b ${config.borderColor}`}>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <span className="text-2xl">{diseaseIcons[safeThreat.disease] || '🔬'}</span>
            <div>
              <h2 className="text-lg font-bold">{safeThreat.disease}</h2>
        ...
        <div className="mt-3 space-y-1 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span>{safeThreat.district}, {safeThreat.region}</span>
          </div>
          <div className="flex items-center gap-2">
            <Calendar className="h-4 w-4" />
            <span>Сезон: {safeThreat.season}</span>
          </div>
          <div className="flex items-center gap-2">
            <Clock className="h-4 w-4" />
            <span>{isReal ? 'Дата вспышки' : 'Обновлено'}: {safeThreat.lastUpdate}</span>
            {isReal && (
        ...
        <div className="p-4 space-y-5">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {safeThreat.affectedAnimals.map(animal => (
          ...
          <div>
            ...
            <div className="space-y-3">
              {safeThreat.recommendations.map((rec, i) => {
                const pConfig = priorityConfig[rec.priority as keyof typeof priorityConfig] || priorityConfig.planned;
                return (
          ...
          <div>
            ...
            <ol className="space-y-2">
              {safeThreat.preventionSteps.map((step, i) => (
          ...
          {safeThreat.isRealData && safeThreat.dataSources.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold flex items-center gap-2 mb-3">
              ...
              <div className="space-y-2">
                {safeThreat.dataSources.map((ds, i) => {
                  const typeConfig = {
                  ...
                  const tc = typeConfig[ds.type as keyof typeof typeConfig] || typeConfig.official;
                  return (
          ...
          <div>
            ...
            <div className="space-y-1">
              {safeThreat.sources.map((source, i) => (
