...
export type DiseaseKey =
  ...
  // ─── Дополнительно (Приказ МСХ №62 + WOAH) ───
  | "sheep_pox" // Оспа овец
  | "goat_pox" // Оспа коз
  | "camel_pox" // Оспа верблюдов
  | "rinderpest" // Чума КРС (искоренена, но в реестре)
  | "contagious_ecthyma" // Контагиозный пустулёзный дерматит (эктима)
  | "malignant_catarrhal_fever" // Злокачественная катаральная горячка
  | "infectious_bovine_rhinotracheitis" // ИРТ КРС
  | "bovine_viral_diarrhea" // Вирусная диарея КРС
  | "parainfluenza3" // Парагрипп-3 КРС
  | "prrs" // Репродуктивно-респираторный синдром свиней
  | "swine_erysipelas" // Рожа свиней
  | "aesv" // Африканская чума лошадей (African horse sickness)
  | "glanders" // Сап
  | "equine_influenza" // Грипп лошадей
  | "equine_viral_arteritis" // Вирусный артериит лошадей
  | "fowl_cholera" // Пастереллёз птиц
  | "pullorum" // Тиф-пуллороз птиц
  | "infectious_bursal_disease" // Инфекционный бурсит (Гамборо)
  | "marek" // Болезнь Марека
  | "infectious_bronchitis" // Инфекционный бронхит кур
  | "infectious_laryngotracheitis" // Инфекционный ларинготрахеит
  | "blackleg" // Эмфизематозный карбункул (эмкар)
  | "enterotoxemia" // Энтеротоксемия
  | "listeriosis" // Листериоз
  | "q_fever" // Лихорадка Ку
  | "tularaemia" // Туляремия
  | "echinococcosis" // Эхинококкоз
  | "cysticercosis" // Цистицеркоз
  | "toxoplasmosis" // Токсоплазмоз
  | "paratuberculosis" // Паратуберкулёз
  | "other";
